...
{
    if (M(Squ1_SfxLenCounter) != 6) // check for second part of bump sound
    {
        DecJpFPS();
        return;
    }
...
{
    if (M(Squ2_SfxLenCounter) != 24) // check for time to play second part
    {
        DecrementSfx2Length();
        return;
    }
...
{
    if (M(Squ2_SfxLenCounter) != 8) // check for almost near the end
    {
        DecrementSfx2Length();
        return;
    }
