...
bool SMBEngine::SpawnHammerObj()
{
    ...
    if (M(Misc_State + miscOffset) != 0) { return false; }
    // NoHammer: leave if the enemy buffer flag at the slot this hammer would use is set
    if (M(Enemy_Flag + HammerEnemyOfsData_data[miscOffset]) != 0) { return false; }
...
std::tuple<uint8_t, uint8_t, uint8_t> SMBEngine::GetFirebarPosition(uint8_t spinstateHigh, uint8_t firebarPart)
{
    ...
    if (hAdder >= 9) { hAdder = (hAdder ^ 0b00001111) + 1; } // GetHAdder: keep result, modified or not, here
    ...
    uint8_t vAdder = (spinstateHigh + 8) & 0b00001111;
    if (vAdder >= 9)
    {
        vAdder = (vAdder ^ 0b00001111) + 1; // otherwise get two's compliment
...
std::pair<uint8_t, uint8_t> SMBEngine::SetupPlatformRope(uint8_t vertSpeed, uint8_t e)
{
    ...
    if (secondaryHardMode_ == 0) { wide = LOBYTE(wide) + 0x10; } // GetLRp
    ...
    if ((vertSpeed & 0x80) != 0) { vertCoord += 8; } // GetHRp
...
uint8_t SMBEngine::PlayerLakituDiff(uint8_t e)
{
    ...
    {
        valueIndex = 1;
        if (player_X_Speed_ >= 0x19 && scrollAmount_ >= 0x02) { valueIndex = 2; }
        ...
        if (!spinyWithPlayerMoving && M(Enemy_Y_Speed + e) == 0) { valueIndex = 0; }
    }
...
void SMBEngine::EndFrenzy(uint8_t e)
{
    ...
    for (uint8_t slot = 5; (slot & 0x80) == 0; --slot)
    {
...
void SMBEngine::MovePiranhaPlant(uint8_t e)
{
    ...
    const auto movePlant = [&]()
    {
        ...
        if (M(Enemy_State + e) != 0) { return; }
        // check enemy's timer here; leave if not yet expired
        if (M(EnemyFrameTimer + e) != 0) { return; }
        // check movement flag
...
// Inputs: e = enemy object buffer offset
// Outputs: none
void SMBEngine::TallBBox2(uint8_t e) { SetBBox2(3, e); }
...
{
    SetBBox(9, e); // unconditional branch
}
...
// Inputs: e = enemy object buffer offset
// Outputs: none
void SMBEngine::TallBBox(uint8_t e) { SetBBox(3, e); }
...
void SMBEngine::MoveRedPTroopa(uint8_t moveDirection, uint8_t e)
{
    ...
    ImposeGravity(moveDirection, e + 1, 3, 6, 2);
}
...
// Outputs: none
void SMBEngine::MovePlatformDown(uint8_t e) { MovePlatform(0, e); }

//------------------------------------------------------------------------

// Inputs: e = enemy object buffer offset (forwarded to MovePlatform)
// Outputs: none
void SMBEngine::MovePlatformUp(uint8_t e) { MovePlatform(1, e); }
...
void SMBEngine::MovePlatform(uint8_t moveDirection, uint8_t e)
{
    ...
    ImposeGravity(moveDirection, e + 1, downwardAmount, 10, 3);
}
...
void SMBEngine::InitFlyingCheepCheep(uint8_t e)
{
    ...
    uint8_t speedSeed = 0;
    if (playerSpeed != 0) { speedSeed = (playerSpeed < 0x19) ? 0x04 : 0x08; }
...
void SMBEngine::XMoveCntr_Platform(uint8_t maxSecondary, uint8_t e)
{
    ...
    if ((frameCounter_ & 0b00000011) != 0) { return; }
...
uint8_t SMBEngine::MoveWithXMCntrs(uint8_t e)
{
    ...
    uint8_t movingDir = 1; // set value here by default
...
void SMBEngine::RunStarFlagObj(uint8_t e)
{
    ...
    case 4:
        ...
        if (M(EnemyIntervalTimer + e) == 0 && eventMusicBuffer_ == 0) { incrementTask(); }
...
void SMBEngine::MoveLiftPlatforms(uint8_t e)
{
    ...
    if (timerControl_ != 0) // if master timer control set, skip all of this
    {
...
void SMBEngine::SmallPlatformCollision()
{
    ...
    const auto findCollision = [&]() -> bool
    {
        ...
        if (CheckPlayerVertical()) { return false; }
        boundBoxCounter = 2; // load counter here for 2 bounding boxes
        ...
        do // ChkSmallPlatLoop
        {
            ...
            if (M(BoundingBox_UL_YPos + boundBoxOfs) >= 0x20)
            {
                ...
                if (PlayerCollisionCore(boundBoxOfs)) { return true; }
            }
...
void SMBEngine::JumpspringHandler(uint8_t e)
{
    ...
    const bool animating = (timerControl_ == 0) && (jumpspringAnimCtrl_ != 0);
    if (animating)
    {
        ...
        if (frameCtrl >= 1)
        {
            ...
            if (aPressed != 0 && (aPressed & previousAbButtons_) == 0) { jumpspringForce_ = 0xf4; }
        }
...
void SMBEngine::DrawPowerUp()
{
    ...
    uint8_t rowsLeft = 1;              // counter to draw two rows of sprite object
    const uint8_t flipBits = 1;        // init d1 of flip control

    do // PUpDrawLoop
    {
        // load left and right tiles of power-up object and draw one row of it
        std::tie(gfxOfs, oamSlot) = DrawOneSpriteRow(PowerUpGfxTable_data[gfxOfs], PowerUpGfxTable_data[1 + gfxOfs], gfxOfs, oamSlot,
                                                     flipBits, attributes, relXPos, yPos);
    ...
    const bool cyclesColors = (powerUpType != 0) && (powerUpType != 3);
    if (cyclesColors)
    {
...
void SMBEngine::BalancePlatform(uint8_t e)
{
    ...
    {
        if (0x2d < M(Enemy_Y_Position + which)) { return false; }
    ...
    if (checkBreak(e, otherPlatform)) { return; }
    if (checkBreak(otherPlatform, e)) { return; }
    ...
    { // branch if collision
        ...
        const uint32_t wide = ((M(Enemy_Y_Speed + e) << 8) | M(Enemy_Y_MoveForce + e)) + 5;
...
void SMBEngine::DrawLargePlatform(uint8_t e)
{
    ...
    {
        if ((offscreenBits & 0x80) != 0) { ram[Sprite_Y_Position + (sprite * 4) + oamOffset] = 0xf8; }
        offscreenBits <<= 1;
    }
...
{
    XMoveCntr_Platform(14, e); // do a sub to increment counters for movement, with the preset
    ...
    if ((M(PlatformCollisionFlag + e) & 0x80) != 0) { return; }
...
{
    NotMoveEnemySlowVert(e, 15); // set movement amount for bowser/other objects
}

//------------------------------------------------------------------------

// Inputs: x = enemy object buffer offset; y = movement amount preset by the caller (0x7f for
// drop platforms, 15 for bowser/other objects)
// Outputs: none
void SMBEngine::NotMoveEnemySlowVert(uint8_t e, uint8_t downwardMoveAmt) { SetXMoveAmt(2, e, downwardMoveAmt); }
...
void SMBEngine::MoveSwimmingCheepCheep(uint8_t e)
{
    ...
    uint8_t preset = SwimCCXMoveData_data[M(Enemy_ID + e) - 10]; // load value here
    ...
    const bool movingDown = M(CheepCheepMoveMFlag + e) >= 16; // check movement flag
    ...
    uint8_t movementSpeed = 0; // load movement speed to upwards by default
    if ((diff & 0x80) != 0)
...
{
    for (int slot = 4; slot >= 0; --slot) // start with last enemy slot
    {
...
void SMBEngine::LargePlatformCollision(uint8_t e)
{
    ...
    if (timerControl_ != 0) // check master timer control
    {
        return; // leave
    }
...
void SMBEngine::EnemiesCollision(uint8_t e)
{
    ...
    if (firstEnemyId >= 0x15 || firstEnemyId == Lakitu || firstEnemyId == PiranhaPlant) { return; }
    ...
    const auto checkEnemyPair = [&](uint8_t second)
    {
        ...
        if (secondEnemyId >= 0x15 || secondEnemyId == Lakitu || secondEnemyId == PiranhaPlant) { return; }
...
// other enemy's offset (also available via zero-page 1)
// Outputs: none
void SMBEngine::ProcEnemyCollisions(uint8_t first, uint8_t second)
{
    ...
    if (M(Enemy_State + first) >= 6)
    {
        ...
        SetupFloateyNumber(M(ShellChainCounter + first) + 4, second); // award appropriate number of points for second enemy
    ...
    if (M(Enemy_State + second) >= 6)
    {
        // check second enemy identifier for hammer bro
        if (M(Enemy_ID + second) == HammerBro) { return; }
        ShellOrBlockDefeat(first); // otherwise, kill first enemy
        // get chain counter for shell, add four to get appropriate point offset
        SetupFloateyNumber(M(ShellChainCounter + second) + 4, first); // award appropriate number of points for first enemy
...
uint8_t SMBEngine::DrawFirebar_Collision(uint8_t oamOffset, uint8_t mirrorData, uint8_t horizAdder, uint8_t vertAdder)
{
    ...
    mirrorBits >>= 1;
    if (!shiftedBit) { horizontal = (horizontal ^ 0xff) + 0x01; } // AddHA: add horizontal coordinate relative to screen to
    ...
    if (!tooFarApart && enemy_Rel_YPos_ != 0xf8)
    {
        ...
        mirrorBits >>= 1;
        if (!shiftedBit) { vertical = (vertical ^ 0xff) + 0x01; } // AddVA: add vertical coordinate relative to screen to
...
uint8_t SMBEngine::FirebarCollision(uint8_t oamOffset, uint8_t segmentX, uint8_t segmentY)
{
    ...
    const auto checkCollision = [&]()
    {
        ...
        uint8_t modX = 0; // always set before the loop can break
...
void SMBEngine::RunBowser(uint8_t e)
{
    ...
    const auto controlBowser = [&]()
    {
        ...
        if ((bowserBodyControls_ & 0x80) != 0) { return; }
        ...
        if ((frameCounter_ & 0b00001111) == 0) { ram[Enemy_MovingDir + e] = 2; }
        ...
        if (M(Enemy_MovingDir + e) == 1) { return; }
...
void SMBEngine::ChkLak(uint8_t startSlot, uint8_t spinySlot)
{
    ...
    while (M(Enemy_ID + lakituSlot) != Lakitu)
    {
        ...
        if ((lakituSlot & 0x80) != 0)
        {
            ...
            ChkNoEn(4); // start with the last enemy slot again
            return;
        }
    }

    // CreateSpiny: if player above a certain point, branch to leave
    if (player_Y_Position_ < 0x2c) { return; }
    // if lakitu is not in normal state, branch to leave
    if (M(Enemy_State + lakituSlot) != 0) { return; }
...
void SMBEngine::PowerUpObjHandler()
{
    ...
    if ((powerUpState & 0x80) != 0)
    {
        ...
        {
            MoveNormalEnemy(5);
            EnemyToBGCollisionDet(5); // deal with collisions
            RunPUSubs(5);             // run the other subroutines
            return;
        }
        if (powerUpType != 2)
        {
            RunPUSubs(5); // if not star, branch elsewhere to skip movement
            return;
        }
        MoveJumpingEnemy(5); // otherwise impose gravity on star power-up and make it jump
        EnemyJump(5);        // note that green paratroopa shares the same code here
        RunPUSubs(5);        // then jump to other power-up subroutines
        return;
    }
...
void SMBEngine::RunNormalEnemies(uint8_t e)
{
    ...
    EnemiesCollision(e);
    PlayerEnemyCollision(e);
    if (timerControl_ == 0) // if master timer control set, skip to last routine
    {
        EnemyMovementSubs(e);
    } // SkipMove
    OffscreenBoundsCheck(e);
...
void SMBEngine::ProcHammerBro(uint8_t e)
{
    ...
    if ((M(Enemy_State + e) & 0b00000111) == 1)
    {
...
void SMBEngine::SetHJ(uint8_t verticalSpeed, uint8_t e, uint8_t jumpLengthBitmask)
{
    ...
    if (secondaryHardMode_ == 0) { jumpLengthIndex = 0; } // HJump: get jump length timer data using offset from before
...
void SMBEngine::MoveHammerBroXDir(uint8_t e)
{
    ...
    if ((diff & 0x80) != 0)
    {
        SetShim(1, e); // if enemy to the left of player, face right
        return;
    }
    // check walking timer
    if (M(EnemyIntervalTimer + e) != 0)
...
void SMBEngine::MoveNormalEnemy(uint8_t e)
{
    ...
    if (stunnedState == 5 || stunnedState < 3)
    {
...
void SMBEngine::EnemyToBGCollisionDet(uint8_t e)
{
    ...
    if ((M(Enemy_State + e) & 0b00100000) != 0) { return; }
    // otherwise, do a subroutine here; leave if enemy vertical coord + 62 < 68
    if (!SubtEnemyYPos(e)) { return; }

    const uint8_t enemyId = M(Enemy_ID + e);
    if (enemyId == Spiny && M(Enemy_Y_Position + e) < 0x25) { return; }
    // DoIDCheckBGColl
    ...
    if (enemyId != Spiny && enemyId != PowerUpObject && enemyId >= 7) { return; }
    ...
    if (static_cast<uint8_t>(vertNybble - 8) >= 5)
    {
        chkForRedKoopa();
        return;
    }
...
void SMBEngine::DoEnemySideCheck(uint8_t e)
{
    if (M(Enemy_Y_Position + e) >= 0x20) // if enemy within status bar, branch to leave
    {
        ...
        for (uint8_t cornerIdx = 22; cornerIdx < 24; ++cornerIdx) // SdeCLoop
        {
            ...
            if (M(0xeb) == M(Enemy_MovingDir + e))
            {
                ...
                const uint8_t metatile = BlockBufferChk_Enemy(1, cornerIdx, e).metatile;
...
void SMBEngine::EnemyJump(uint8_t e)
{
    ...
    const auto jump = [&]()
    {
        ...
        if (static_cast<uint8_t>(M(Enemy_Y_Speed + e) + 2) < 3) { return; }
...
void SMBEngine::EnemiesAndLoopsCore(uint8_t enemyOffset)
{
    ...
    if ((enemyFlag & 0x80) != 0)
    {
        ...
        if (M(Enemy_Flag + otherFlagOffset) != 0) { return; }
    ...
    if (enemyFlag == 0)
    {
        ...
        if ((areaParserTaskNum_ & 0x07) == 7) { return; }
    ...
    while (true) // ProcLoopCommand
    {
        ...
        if (loopCommand_ != 0 && currentColumnPos_ == 0)
        {
            ...
            for (int loopIndex = 10; loopIndex >= 0; --loopIndex) // FindLoop
            {
                ...
                if (worldNumber_ != LoopCmdWorldNumber_data[loopIndex]) { continue; }
                // check to see if one of the page numbers
                if (currentPageLoc_ != LoopCmdPageNumber_data[loopIndex]) { continue; }
        ...
        if ((firstByte & 0b00001111) != 14 && e >= 5)
        {
        ...
        if (enemyId == Goomba && primaryHardMode_ != 0) { enemyId = BuzzyBeetle; }
    ...
    const auto bulletBillCheepCheep = [&]()
    {
        ...
        if (areaType_ != 0) // are we in a water-type level?
        {
            ...
            for (uint8_t slot = 0; slot < 5; ++slot) // BB_SLoop: move onto the next slot
            {
                ...
                if (M(Enemy_Flag + slot) == 0) { continue; }
        ...
        else
        {
            ...
            uint8_t ccOfs = 0; // load default offset
...
void SMBEngine::HandleGroupEnemies(uint8_t enemyByte)
{
    ...
    uint8_t enemyId = 0; // load value for green koopa troopa
    if (groupIndex < 4)
    {
    ...
    while (enemiesLeft)
    {
        ...
        uint8_t slot = 0;
        while (slot < 5 && M(Enemy_Flag + slot) != 0)
        {
            ++slot;
        }
        if (slot >= 5)
