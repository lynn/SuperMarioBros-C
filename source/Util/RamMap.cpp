...
const std::size_t ramMapLength = sizeof(ramMap) / sizeof(ramMap[0]);

// The colours are the ones the regions are asked for, lightened enough to read
// against the dark background of the view. The two blues are set apart from one
// another, the stack being the duller of them.
//
const RamRegion ramRegions[] = {
    {"Scratch",      0x0000, 0x0007, 0xc89050}, // brown
    {"Stack",        0x0100, 0x01ff, 0x5a6fc8}, // dark blue
    {"OAM",          0x0200, 0x02ff, 0xc89050}, // brown
    {"Hitboxes",     0x04ac, 0x04ff, 0xb478f0}, // purple
    {"Block buffer", 0x0500, 0x069f, 0x5ab4ff}, // blue
    {"Timers",       0x0780, 0x07a2, 0x78d278}, // green
};

const std::size_t ramRegionsLength = sizeof(ramRegions) / sizeof(ramRegions[0]);

const RamRegion* findRamRegion(uint16_t address)
{
    for (std::size_t i = 0; i < ramRegionsLength; i++)
    {
        if (address >= ramRegions[i].first && address <= ramRegions[i].last)
        {
            return &ramRegions[i];
        }
    }
